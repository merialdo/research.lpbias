...
class Dataset:
    ...
    def __init__(self,
        ...
        # map each relation to its train triples
        self.relation_2_train_triples = defaultdict(lambda: set())
        ...
        if load:
            ...
            for i in range(len(self.train_samples)):
                (h_id, r_id, t_id) = self.train_samples[i]
                self.train_samples_set.add((h_id, r_id, t_id))

                (h_name, r_name, t_name) = self.train_triples[i]
                self.train_triples_set.add((h_name, r_name, t_name))
                self.relation_2_train_triples[r_name].add((h_name, r_name, t_name))
    ...
    def _compute_relation_2_type(self):
        ...
        for relation in relation_2_heads_nums:
            ...
            relation_name = self.relation_id_2_name[relation]

            if average_heads_per_tail > 1.2 and average_tails_per_head > 1.2:
                self.relation_2_type[relation_name] = MANY_TO_MANY
            elif average_heads_per_tail > 1.2 and average_tails_per_head <= 1.2:
                self.relation_2_type[relation_name] = MANY_TO_ONE
            elif average_heads_per_tail <= 1.2 and average_tails_per_head > 1.2:
                self.relation_2_type[relation_name] = ONE_TO_MANY
            else:
                self.relation_2_type[relation_name] = ONE_TO_ONE
